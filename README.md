...
The frontend immediately renders the last saved live dashboard data from the browser cache, or the saved fallback schedule and member list when no live cache exists yet, before it checks Google Apps Script. This keeps the site usable on first open even when Apps Script is cold-starting, slow, blocked, or has not been redeployed yet. Dashboard requests use a longer timeout plus one retry so users do not need to upload a payment just to warm the backend. If every live-data attempt fails, the page keeps the cached live dashboard when available instead of replacing it with zero-payment fallback data; otherwise it shows the default ₱50 weekly schedule, zero recorded payments, and a warning that live Google Sheets data is unavailable. Payment uploads still run the backend health check before submitting, so fix the deployed Apps Script URL/access settings if uploads fail.
