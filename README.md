...
2. Records are currently targeted to the existing `Payment Tracker Data` spreadsheet ID configured in `SPREADSHEET_ID` in `code.gs`. The spreadsheet ID is the text between `/d/` and `/edit` in the Sheet URL. If you intentionally switch to a different Sheet, replace that ID and deploy a new web app version. If `SPREADSHEET_ID` is blank, the backend creates a separate `Payment Tracker Data` spreadsheet automatically on the first successful payment upload and reuses that spreadsheet on future uploads.
...
After a successful upload, the website shows an **Open recorded sheet** link returned by the backend. Use that link to confirm the record went to the intended spreadsheet and `Payments` tab.
