...
7. To enable automatic member emails, run `installContributionReminderTrigger` once in Apps Script and approve the requested email/trigger permissions. This creates a daily trigger for `sendContributionReminderEmails`, which sends: a reminder exactly 3 days before each due date, a due-today email on the exact due date, and an overdue notice after missed due dates. The email job uses each member's accumulated total contribution: it skips a member when their total recorded payments are already equal to or greater than the cumulative target for that due date, such as ₱50 by week 1, ₱100 by week 2, and ₱150 by week 3 when the weekly amount is ₱50.
...
9. Confirm the deployed web app URL in `APPS_SCRIPT_URL` in `script.js` is current. The current URL is `https://script.google.com/macros/s/AKfycbyNj24bFYPFiEDJYt_6sAuW65yH8CKkZCW_plVT9le5LASFSpH3Zf9H0ZnL6X8VMGkw/exec`.

The website sends submissions through a hidden HTML form/iframe as a browser-safe form payload to avoid Google Apps Script CORS/redirect issues that can prevent `fetch(..., { mode: "no-cors" })` posts from reliably reaching `doPost`. Each submission now includes a unique `submissionId`; after sending, the page polls the Apps Script `doGet?action=status` endpoint through JSONP until it verifies that the row exists in the `Payments` sheet. Transient JSONP verification timeouts are retried with a longer timeout window, because Apps Script can be slow while saving receipt files or warming up. This prevents the page from showing a false success when Apps Script accepted the browser request but did not write the row, while also avoiding immediate false redeploy errors for slow responses. `index.html` loads `script.js` with a cache-busting query string so GitHub Pages and the browser do not keep running an old script. The frontend treats known-compatible Apps Script versions as valid and only blocks when required dashboard fields or sheet headers are missing, so a newer backend version does not stop dashboard data from loading just because the version label changed.
